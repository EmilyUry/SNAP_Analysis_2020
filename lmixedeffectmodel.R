...
model1 <- lmer(DOC~logCl + logSO4 + pH + (1|Site), data = D, REML = F)
null1 <- lmer(DOC~1 + (1|Site), data = D, REML = F)
anova(null1, model1)
summary(model2)


model1 <- lmer(DOC~logCl + logSO4 + pH + SM + (1|Site), data = D, REML = F)
null1 <- lmer(DOC~1 + (1|Site), data = D, REML = F)
anova(null1, model1)
summary(model1)
...
### carb
model1 <- lmer(carb~logCl + logSO4 + pH + (1|Site), data = D, REML = F)
null1 <- lmer(carb~1 + (1|Site), data = D, REML = F)
anova(null1, model1)
summary(model2)
...
### Cmin
model1 <- lmer(Cmin_c~logCl + logSO4 + pH + (1|Site), data = d, REML = F)
null1 <- lmer(Cmin_c~1 + (1|Site), data = d, REML = F)
anova(null1, model1)
summary(model2)
...
### Cmin
model1 <- lmer(Phenol~logCl + logSO4 + pH + (1|Site), data = D, REML = F)
null1 <- lmer(Phenol~1 + (1|Site), data = D, REML = F)
anova(null1, model1)
summary(model2)
