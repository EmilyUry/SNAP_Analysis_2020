...
MZ <- lmer(DOC~ logCl*pH + (1 | Site/Date) , data = Z, REML = F)
Mz <- lmer(DOC~logCl + logCl*pH + (1 | Site/Date), data = z, REML = F)
summary(MZ)
summary(Mz)


MZ <- lmer(carb~logCl + pH + logCl*pH + (1 | Site/Date) , data = Z, REML = F)
Mz <- lmer(carb~logCl + pH + logCl*pH + (1 | Site/Date), data = z, REML = F)
summary(MZ)
summary(Mz)

options(na.action = "na.omit")
MZ <- lmer(Cmin_c~logCl + pH + logCl*pH + (1 | Site/Date) , data = Z, REML = F)
Mz <- lmer(Cmin_c~logCl + pH + logCl*pH + (1 | Site/Date), data = z, REML = F)
summary(MZ)
summary(Mz)


MZ <- lmer(Phenol~logCl + pH + logCl*pH + (1 | Site/Date) , data = Z, REML = F)
Mz <- lmer(Phenol~logCl + pH + logCl*pH + (1 | Site/Date), data = z, REML = F)
summary(MZ)
summary(Mz)
